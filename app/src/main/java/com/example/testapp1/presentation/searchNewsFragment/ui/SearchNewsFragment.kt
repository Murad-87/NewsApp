...
class SearchNewsFragment :
    ...
    private fun handleSuccess(response: Resource<NewsDataResponse>) {
        ...
        response.data?.let { newsDataResponse ->
            newsAdapter.submitList(newsDataResponse.results.toList())
            val totalPages = newsDataResponse.totalResults / QUERY_PAGE_SIZE + 2
    ...
    private fun navigate(article: NewArticleRemote) {
